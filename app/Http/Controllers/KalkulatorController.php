...
class KalkulatorController extends Controller
{
    ...
    //         $request->validate([
    //             'jenis_zakat_id' => 'required|exists:jenis_zakat,id',
    //             'nilai_harta' => 'required|numeric|min:0',
    //         ]);

    //         $jenisZakat = JenisZakat::find($request->jenis_zakat_id);
    //         $nilaiHarta = (float) $request->nilai_harta;

    //         $hargaEmas = (float) Setting::where('setting_key', 'harga_emas')->value('setting_value');

    //         // ## PERUBAHAN LOGIKA NISAB DI SINI ##
    //         $nisab = 0;
    //         $wajibZakat = false;
    //         $nominalZakat = 0;

    //         // Logika khusus untuk Zakat Profesi
    //         if (str_contains(strtolower($jenisZakat->name), 'profesi')) {
    //             // Nisab zakat profesi adalah per bulan, setara 1/12 dari 85 gram emas
    //             $nisabBulanan = ($jenisZakat->nisab_quantity * $hargaEmas) / 12;
    //             $nisab = $nisabBulanan; // Set nisab untuk dikirim ke frontend

    //             // Bandingkan pendapatan per bulan dengan nisab bulanan
    //             if ($nilaiHarta >= $nisabBulanan) {
    //                 $wajibZakat = true;
    //                 $nominalZakat = ($jenisZakat->rate_percent / 100) * $nilaiHarta;
    //             }
    //         }
    //         // Logika untuk Zakat Maal lainnya (Emas, Simpanan, dll)
    //         else {
    //             // Nisab zakat maal adalah tahunan, setara 85 gram emas
    //             $nisabTahunan = $jenisZakat->nisab_quantity * $hargaEmas;
    //             $nisab = $nisabTahunan; // Set nisab untuk dikirim ke frontend

    //             // Bandingkan total harta dengan nisab tahunan
    //             if ($nilaiHarta >= $nisabTahunan) {
    //                 $wajibZakat = true;
    //                 $nominalZakat = ($jenisZakat->rate_percent / 100) * $nilaiHarta;
    //             }
    //         }
