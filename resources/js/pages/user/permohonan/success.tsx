// resources/js/Pages/user/permohonan/success.jsx (Dengan Perbaikan)

import { Button } from '@/components/ui/button';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import PublicLayout from '@/layouts/publicLayout';
import { Head, Link } from '@inertiajs/react'; // <-- usePage dihapus karena tidak perlu
import { Check, CheckCircle, Copy, Search } from 'lucide-react';
import { useEffect, useState } from 'react';
import Confetti from 'react-confetti';
import { toast, Toaster } from 'sonner';

// ## PERUBAHAN #1: Terima `unique_code` sebagai props langsung ##
export default function Success({ unique_code }) {
    ...
    return (
        <PublicLayout>
            ...
            <section className="container mx-auto -mt-10 max-w-2xl px-6 pb-24">
                <Card className="text-center shadow-lg duration-700 animate-in fade-in slide-in-from-bottom-5">
                    <CardHeader>
                        ...
                        <p className="pt-2 text-muted-foreground">
                            ...
                            permohonan Anda. Mohon catat atau simpan di tempat
                            yang aman.
                        </p>
                    </CardHeader>
                    <CardContent className="flex flex-col items-center">
                        <div className="my-6 w-full max-w-md rounded-lg border-2 border-dashed bg-gray-50/80 p-6">
                            <p className="text-base font-semibold text-muted-foreground">
                                Kode Unik Anda:
                            </p>

                            {/* ## PERUBAHAN #2: `unique_code` sekarang akan tampil dengan benar ## */}
                            <p className="mt-2 text-3xl font-extrabold tracking-wider text-primary">
                                {unique_code}
                            </p>
                        </div>
                        <Button
